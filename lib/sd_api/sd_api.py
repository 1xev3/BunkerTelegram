...
class APIQueue():
    ...
    async def put(self, params:Params):
        ...
        if self.counts[uid] >= max_count:
            raise MaxQueueReached("Максимальное количество одновременных запросов достигнуто")
...
class WebUIApi():
    ...
    def configure(self, host = "localhost", port = 7860):
        self.host = host
        self.port = port
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            api.update_models()
            pprint(api.get_models())

            prompt = "a beautiful girl"
            negative_prompt = ""

            params = txt2img_params()
            params.prompt = f"safe, masterpiece, best quality, amazing quality, {prompt}"
            params.negative_prompt = f"nsfw, {negative_prompt}, Base:bad quality,worst quality,worst detail,sketch,censor, signature, curvy, realistic"
            params.width = 832
            params.height = 1216
            params.batch_size = 1
            params.n_iter = 1
